...
| 2 | [`traces/m2-critic-rejection.txt`](traces/m2-critic-rejection.txt) | The critic rejects a draft on three counts, including a claim that stories were queued for sprint planning when `propose_stories` was never called. Fail-action fires, revision 1 of 2, then escalates. Captured before the per-check verdict schema landed, so it shows a reasons list; the schema is described in `03-orchestration/orchestration-map.md` Field 5 and was verified separately against a deliberately bad draft. | M3 |
