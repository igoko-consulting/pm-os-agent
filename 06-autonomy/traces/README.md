...
Still to capture: a bound trip (item 4) and a queue-cap rejection (item 5), both from M5. Also still
uncovered: an empty week. Every fixture that could reach it escalates on an earlier blocker first, so it
needs a project that is quiet but not confidential, which the fixture data does not currently have.
